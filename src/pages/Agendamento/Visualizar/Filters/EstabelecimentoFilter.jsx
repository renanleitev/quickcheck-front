...
import { getHorariosByEstabelecimentoAndStatus } from '../../../../store/modules/horarios/reducer';

const buttonWidth = '12rem';

EstabelecimentoFilter.propTypes = {
  data: PropTypes.object.isRequired,
  initialData: PropTypes.object.isRequired,
  setData: PropTypes.func.isRequired
};

export default function EstabelecimentoFilter({ data, initialData, setData }) {
  const [open, setOpen] = useState(false);

  const dispatch = useDispatch();

  const handleSearch = () => {
    dispatch(getHorariosByEstabelecimentoAndStatus({ ...data }));
    setOpen(false);
    toast.success('Pesquisa efetuada com sucesso!');
  };

  const handleReset = () => {
    dispatch(getHorariosByEstabelecimentoAndStatus({ ...initialData }));
    setData({ ...initialData });
    setOpen(false);
